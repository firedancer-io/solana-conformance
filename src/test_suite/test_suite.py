...
@app.command(help=f"Execute {globals.harness_ctx.context_type.__name__} message(s) and print the effects.")
def exec_instr(
...
@app.command(help=f"Extract {globals.harness_ctx.context_type.__name__} messages from fixtures.")
...
@app.command(help=f"Convert {globals.harness_ctx.context_type.__name__} messages to human-readable format.")
def decode_protobuf(
    input_dir: Path = typer.Option(
