...
@app.command(help=f"Execute message(s) and print the effects.")
def exec_instr(
    file_or_dir: Path = typer.Option(
        None,
        "--input",
        "-i",
        help=f"Input file or directory of files",
    ),
...
@app.command(help=f"Extract messages from fixtures.")
...
@app.command(help=f"Convert messages to human-readable format.")
def decode_protobuf(
    input_path: Path = typer.Option(
        Path("raw_context"),
        "--input",
        "-i",
        help=f"Either a message or directory of messages",
    ),
    output_dir: Path = typer.Option(
        Path("readable_context"),
        "--output-dir",
        "-o",
        help=f"Output directory for base58-encoded, human-readable messages",
    ),
    num_processes: int = typer.Option(
        4, "--num-processes", "-p", help="Number of processes to use"
    ),
